...
private:
    ...
    void predict_future_ped_states(const pedestrian_msgs::PeopleStates& current_people, pedestrian_msgs::PeopleStates& future_people, ros::Time now);  // 歩行者の将来位置を予測
    ...
    // yamlファイルで設定可能な変数
    int hz_;                               // ループ周波数 [Hz]
    bool visualize_current_people_poses_;  // 現在の歩行者の位置を可視化するかの設定用
